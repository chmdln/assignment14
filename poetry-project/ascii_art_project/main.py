...
class AsciiArtGenerator:
    ...
    def load_ascii_art(self): 
        file_name = f"./ascii_art_project/data/{self.args.banner}.txt"
    ...
    def process_text(self): 
        words = self.args.text 
        words = bytes(self.args.text, "utf-8").decode("unicode_escape")
        if words == "": 
            if self.args.output:
                with open(self.self.args.output, 'w') as f:
                    pass
            sys.exit(0) 
        if words == "\n":
            if self.args.output:
